...
public class DNSLookupServiceTest {
    ...
    private int checkQuery(DNSQuestion question, DNSMessage message, int transactionId) {
        ...
        Assertions.assertEquals(transactionId, receivedTransactionId,
                "Returned transaction ID does not match ID used in query message");
    ...
    private void processResponseCommonTest(boolean authoritative, int errorCode,
        ...
        DNSMessage response = buildResponse(responseID, authoritative, errorCode, questions, answers, nameservers,
                additional);
    ...
    @Test
    public void testIndividualQueryProcess() throws UnknownHostException, DNSLookupService.DNSErrorException {
        DNSQuestion question = new DNSQuestion("www.cs.ubc.ca", RecordType.A, RecordClass.IN);

        InetAddress address = InetAddress.getByName("a.root-servers.net");
        Set<ResourceRecord> ans = service.individualQueryProcess(question, address);
